...
- Engine cho phép mua mới nhưng với quy mô nhỏ (partial entry) – các lệnh mua mới có thể chỉ mua một phần nhỏ (được đánh dấu là PARTIAL_ENTRY). Từ nay, partial entry được điều khiển bởi các tham số chung trong `thresholds` (`partial_entry_enabled`, `partial_entry_frac`, `new_partial_buffer`, `partial_entry_floor_lot`), nên ngay cả khi không ở trạng thái neutral engine vẫn có thể khởi động lệnh NEW_PARTIAL cho các ứng viên đạt gần ngưỡng q_new.
