...
- Mọi chỉnh sửa thủ công sẽ bị ghi đè ở lần chạy kế tiếp. Cần thay đổi hành vi → cập nhật overlay trong `config/` (nightly/publish) hoặc baseline khi có quyết định chiến lược.
...
- Engine đọc config/policy_default.json vào đối tượng default_obj. Nếu tồn tại các overlay (nightly, ai, unified publish) thì lần lượt deep‑merge chúng lên default_obj. Không còn bước lọc khóa tự động – mọi khóa có trong overlay sẽ ghi đè trực tiếp lên baseline.
...
Execution update (2025‑10‑08)
- Lớp xuất lệnh (orders_io) kẹp `LimitPrice` về giá thị trường trong PHIÊN (bao gồm giờ nghỉ trưa) để tránh đặt giá bất lợi:
...
- Chạy `./broker.sh orders` sau tune; in diff overlay (mới vs snapshot) và các file output chính vào log; sau cùng commit & push `config/policy_overrides.json` (nhánh `main`).
