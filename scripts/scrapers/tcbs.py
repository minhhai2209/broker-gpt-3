...
def fetch_tcbs_portfolio(
    ...
    portfolio_dir = (portfolios_dir / profile).resolve()
    portfolio_dir.mkdir(parents=True, exist_ok=True)
    out_path = portfolio_dir / "portfolio.csv"
...
def _fetch_statement_today(page, root: Path, profile: str) -> Tuple[Path, Path]:
    ...
    Writes data/portfolios/<profile>/order_history.csv with columns:
    ...
    dest_dir = (root / "data" / "portfolios" / profile).resolve()
    dest_dir.mkdir(parents=True, exist_ok=True)
    out_path_today = dest_dir / "order_history.csv"
    out_path_all = dest_dir / "order_history_all.csv"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="(Default ON) Also fetch today's executed orders and write data/portfolios/<profile>/order_history.csv",
    )
